...
import requests
import matplotlib
matplotlib.use('Agg') # matplotlib v3.2.1
...
def graphs(symbol,interval):
    ...
    for i in range(0,201):
        if interval == '1d' : unitoftime = 24*60*60
        ...
        x.append(time.strftime("%b%d %Y %H:%M",time.localtime(time.time()-(199-i)*unitoftime)))
    ...
    plt.plot(x,y0,label='Close Price',color='black')
    ...
    plt.xticks((x[0],x[19],x[39],x[59],x[79],x[99],x[119],x[139],x[159],x[179],x[199]), rotation=30)
